...
> focuses mainly on PC parts, prices and compatibility, MyRig focuses on the full gaming
> setup experience. Users select their games, budget, setup style and goals. The app uses
> the RAWG API to search real games, Neon Postgres to store the catalog, accounts, orders
...
`RAWG_API_KEY`, `DATABASE_URL` and `AUTH_SECRET` are set — without them, everything that
...
2. **Can I Run It? (45s).** Search **Cyberpunk 2077**. Say: *"this is a live call to RAWG,
...
3. **Quiz (60s).** Pick **Balanced ($900–$1500)**, **Streaming / content**, **Streamer
...
| External API | RAWG, via `api/games/*` and `api/can-i-run.js` |
...
Read the game's RAWG genres/tags to detect competitive vs graphics vs casual → set a target
...
- **No compatibility checking** — that is PCPartPicker's job, not this one.
...
- The catalog is small (40 products). Enough to show the logic differentiating builds, which
  is the point.
